...
# 5. 可视化函数
def visualize_projection_on_image(cubes, cameras, K, image_folder, image_id=1,is_rotated_180=False):
    """在实际图像上可视化3D立方体的投影"""
    ...
    for cube in cubes:
        ...
        if True:
            ...
            # class_id = cube['label']
            # box_color = class_color_map.get(class_id, (128, 128, 128))  # 类别颜色
            # class_name = cube.get('label_name', class_name_map.get(class_id, f'class_{class_id}'))
...
# 6. 主函数
def main(json_path, camera_txt_path, image_folder,is_rotated_180):
    # 解析数据
    ...
    for image_id in cameras:
        fig = visualize_projection_on_image(cubes, cameras, K, image_folder, image_id, is_rotated_180)
        if True:
            # if image_id >= 71:
            #     plt.show()
...
if __name__ == "__main__":
    # 手动设置文件路径
    json_annotation_path = "/mnt/dln/data/datasets/0915/make_label_raw/label/B_area_1_2-33148.json"  # 3D点云标注JSON文件路径
    camera_extrinsics_path = "/mnt/dln/data/datasets/0915/make_label_raw/B_area_1_2/sparse/0/images.txt"  # 相机外参文件路径
    image_folder_path = "/mnt/dln/data/datasets/0915/make_label_raw/B_area_1_2/fixed_images/"  # 图像文件夹路径
    ...
    # 检查文件是否存在
    main(json_annotation_path, camera_extrinsics_path, image_folder_path,is_rotated_180 = True)
